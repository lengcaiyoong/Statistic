...
STR = function(s_out,s_frame, strata, variable_lvl){
	...
	###Any Non-response? 
	  ...
	  if (ANS1 == "WEIGHTING CLASS" | ANS1 == "Weighting Class" | ANS1 == "weighting class") {
			...
			s_out$BASE_WEIGHT <-  rep(BASE_WEIGHT, nrow(s_out))	
			s_out$BASE0_WEIGHT <-  rep(BASE_WEIGHT, nrow(s_out))	
						#Check Stability of WEIGHT
				stability_list1 <- rep(NA, length(unique(s_out$NEW.VARIABLE))  )
				stability2 <- split(data0, data0$NEW.VARIABLE)
			...
			#Change Identity Status To 1 & 0
			  ...
			  s_out <- cbind(	s_out , as.data.frame(str_storage_varlvl, col.names=variable_lvl))
						#assign 0 to NA rows in 1&0 identity status
				for (i in 1:length(variable_lvl)){
  	 					(select(  s_out, as.character(variable_lvl)  )[,i])[which(is.na(select(  s_out, as.character(variable_lvl)  ) [,i]	))] <- 0
								         }
			#Calculate Nc
			...
			#Random Group Variance
			  ...
			  srsVAR <- RGVAR_SRS(s_out,s_frame, strata, variable_lvl)
				...
				##IMPUTATION##
		if(  ANS1 == "IMPUTATION" | ANS1 == "Imputation" | ANS == "imputation"  ){
...
STR(s_out,s_frame, strata, variable_lvl)