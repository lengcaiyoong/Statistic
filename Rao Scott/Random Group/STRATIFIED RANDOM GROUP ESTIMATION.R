...
#RGVAR_STR(s_out,s_frame, strata, variable_lvl)


RGVAR_STR = function(s_out,s_frame, strata, variable_lvl){
	#Decide Group numbers	
	   ...
	   if(is.prime(min(u_strata)) ){		 #suggest imputation if primed number
	    ...
	    n_rdgrp = readline(noquote("choose NUMBER OF RANDOM GROUP:	"))		#ask user to select number of random group
	#Create Random Group and Base Weight
		nonreplaced_strata = s_out
		nonreplaced_DEPTCODE <- sort(unique(s_out$DEPT_CODE)) 
		Nhmh_nonreplaced <- 
			( (as.numeric(table(s_frame$DEPT_CODE)) [which(s_frame$DEPT_CODE ==   nonreplaced_DEPTCODE)]) / ( min(u_strata)/n_rdgrp )  )						
