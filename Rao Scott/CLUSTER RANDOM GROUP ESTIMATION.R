#RGVAR_CLS(s_out_clswr,s_frame, strata, variable_lvl,n,s_out_rgsrs)




RGVAR_CLS = function(s_out,s_frame, strata, variable_lvl,n,s_out_rgsrs){
	#Define Group Numbers n_rdgrp
		...
		n_rdgrp = as.numeric(readline(noquote("NUMBER OF RANDOM GROUP:	")))
				}
	#Split
		n_cluster = n / n_rdgrp					#number of cluster in each random group
		u_rdgrp = n_cluster * (nrow(s_out)/n)		#number of units in each random group
	...
	#Create BASE WEIGHT	
		...
		#Create Random Group
		  ...
		  for (i in 1:n_rdgrp){			#randomly sample n_rdgrp times by n_cluster
		 	...
		 	rdgrp = list()
		  for (i in 1:n_rdgrp){
		      rdgrp[[i]] <- clscode_storage[cls_stor[[i]]]
						}
		  for (i in 1:length(rdgrp)){		#unlist list within list 
			rdgrp[[i]] <- do.call(rbind.data.frame, rdgrp[[i]])
						}
